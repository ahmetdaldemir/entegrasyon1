...
class aras {	
	...
	function getBarcode($code) {
		try {
			$client = new SoapClient($this->url, array("encoding"=>"UTF-8"));
			$response=$client->GetBarcode(array("Username"=>$this->UserName,"Password"=>$this->Password,"integrationCode"=>$code));
			$return = (array) $response->GetBarcodeResult;
		} catch (Exception $exc) {
			$return = $exc->getMessage();
		}
	...
	function printToLocalZebra($data) {
